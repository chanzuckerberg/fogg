...
type AWSProvider struct {
	...
	DefaultTags *v2.AWSProviderDefaultTags `yaml:"default_tags"`
	IgnoreTags  *v2.AWSProviderIgnoreTags  `yaml:"ignore_tags"`
}
	...
	for envName, envConf := range conf.Envs {
		...
		for componentName, componentConf := range conf.Envs[envName].Components {
			...
			componentPlan.CdktfDependencies = map[string]string{
				...
				"@vincenthsh/cdktf-fogg-helpers": "^1.5.2",
...
func resolveAWSProvider(commons ...v2.Common) (plan *AWSProvider, providers []AWSProvider, version *string) {
	...
	plan = &AWSProvider{
		...
		RoleArn:     roleArn,
		DefaultTags: awsConfig.DefaultTags,
		IgnoreTags:  awsConfig.IgnoreTags,
	}
...
func resolveComponentCommon(fs afero.Fs, path string, commons ...v2.Common) ComponentCommon {
	...
	if sopsConfig != nil && (sopsConfig.Enabled == nil || (sopsConfig.Enabled != nil && *sopsConfig.Enabled)) {
		customProvider := false
		if sopsConfig.CustomProvider != nil {
			customProvider = *sopsConfig.CustomProvider
		}
		version := "0.7.2"
		if sopsConfig.Version != nil {
			version = *sopsConfig.Version
		}
		sopsPlan = &SopsProvider{
			CommonProvider: CommonProvider{
				Version:        version,
				Enabled:        sopsConfig.Enabled == nil || (sopsConfig.Enabled != nil && *sopsConfig.Enabled),
				CustomProvider: customProvider,
			},
		}
