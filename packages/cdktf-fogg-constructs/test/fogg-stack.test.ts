...
import type {
  ...
  ListOutput,
  MapOutput,
} from '../src/util/remote-state-access-proxy'
...
describe('FoggStack', () => {
  ...
  describe('creates remote states if forceRemoteStates is true', () => {
    let stack: FoggStack
    /**
     * helper to add a TerraformOutput to the stack
     */
    function addOutput(outputName: string, value: any) {
      new TerraformOutput(stack, outputName, {
        value,
        staticId: true,
      })
    }
    ...
    // Construct writers can define output schemas
    const endpointTypes = ['apigw', 's3'] as const
    type Endpoints = {
      // immutable string array of endpoint names
      [K in (typeof endpointTypes)[number]]: string
    }
    ...
    interface NetworkOutputs {
      ...
      internal_dns_zone: MapOutput<InternalZone>
      endpoints: MapOutput<Endpoints>
    }
    const networkOutputsSchema: InferOutputSchema<NetworkOutputs> = {
      vpc_id: 'string',
      app_subnet_ids: 'list',
      endpoints: 'map',
      internal_dns_zone: 'map',
    }
    ...
    it('with strongly typed access to simple outputs', () => {
      ...
      addOutput('vpc_id', outputs.vpc_id)
      Template.fromStack(stack).toMatchObject({
        output: {
          vpc_id: {
    ...
    it('with strongly typed access to strange output names', () => {
      interface StrangeOutputs {
        'very-StrangeName': string
      }

      const outputs = stack.remoteState<StrangeOutputs>('network')
      addOutput('strangeName', outputs['very-StrangeName'])
      Template.fromStack(stack).toMatchObject({
        output: {
          strangeName: {
            value:
              '${data.terraform_remote_state.network.outputs.very-StrangeName}',
          },
        },
      })
    })
    // TODO: doesn't work because without schema, proxy returns a string token
    // which doesn't throw when indexed
    it.todo('throws on incorrectly accessing list items', () => {
      const outputs = stack.remoteState<NetworkOutputs>('network')
      addOutput('app_subnet1', outputs.app_subnet_ids[0])
      expect(() => Testing.synth(stack)).toThrow(/Found an encoded/)
    })
    // TODO: To handle this, we need to return a nested proxy... won't implement
    it.todo('throws on incorrectly accessing list items', () => {
      const outputs = stack.remoteState<NetworkOutputs>(
        'network',
        networkOutputsSchema,
      )
      // should use outputs.app_subnet_ids.element(0)
      addOutput('app_subnet1', outputs.app_subnet_ids[0])
      expect(() => Testing.synth(stack)).toThrow(
        /Numeric indexing remote state outputs is not supported/,
      )
    })
    it('with strongly typed access to complex outputs', () => {
      ...
      addOutput('vpc_id', outputs.vpc_id)
      addOutput('app_subnet1', outputs.app_subnet_ids.element(0))
      addOutput('app_subnet2', outputs.app_subnet_ids.element(1))
      addOutput('internal_zone_id', outputs.internal_dns_zone.lookup('id'))
      addOutput('endpoint_apigw', outputs.endpoints.lookup('apigw'))
      addOutput(
        'endpoint_apigw_domain',
        outputs.endpoints.lookupNested('apigw', 'domain'),
      )
      Template.fromStack(stack, { snapshot: false }).toMatchObject({
        output: {
          ...
          internal_zone_id: {
            value:
              '${data.terraform_remote_state.network.outputs.internal_dns_zone.id}',
          },
          endpoint_apigw: {
            value:
