...
describe('FoggTerraStack', () => {
  ...
  it('throws error on unsupported backend', () => {
    ...
    // THEN
    expect(
      () => new FoggTerraStack(app, 'MyStack', { gridUUID, environmentName }),
    ).toThrow(/Unsupported backend configuration/)
  })
  ...
  it('throws for unsupported generic providers', async () => {
    ...
    expect(
      () => new FoggTerraStack(app, 'MyStack', { gridUUID, environmentName }),
    ).toThrow(/Unsupported provider foo\/bar/)
  })
})
