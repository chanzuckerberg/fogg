...
func TestApplyModuleInvocationWithVariables(t *testing.T) {
	r := require.New(t)
	testFs, d, err := util.TestFs()
	r.NoError(err)
	defer os.RemoveAll(d)
	...
	downloader, err := util.MakeDownloader("test-module")
	r.NoError(err)
	e := applyModuleInvocation(fs, "mymodule", "test-module", nil, []string{"foo"}, templates.Templates.ModuleInvocation, templates.Templates.Common, downloader)
	r.NoError(e)
...
func TestApplyModuleInvocationWithModuleName(t *testing.T) {
	...
	downloader, err := util.MakeDownloader("test-module")
	r.NoError(err)
	e := applyModuleInvocation(fs, "mymodule", "test-module", &moduleName, []string{}, templates.Templates.ModuleInvocation, templates.Templates.Common, downloader)
	r.NoError(e)
